...
TEST_FORMAT = 'json'
TEST_BUCKET = 'bucket11'

async def get_user_quota() -> Tuple[HTTPStatus, Dict[str, Any]]:
    """
    ...
    print(f'Request parameters : {user_params}')
    status, body = await rgwcli.signed_http_request(
        'GET', '/admin/user?quota', query_params=user_params)
...
async def get_bucket_quota() -> Tuple[HTTPStatus, Dict[str, Any]]:
    """
    Illustrate S3Client signed_http_request work.
...
if __name__ == "__main__":
    ...
    #status, uiser_info = loop.run_until_complete(get_user_quota())
    status, user_info = loop.run_until_complete(get_bucket_quota())
